...
export class Marquee {
   ...
   * 跑马灯预设样式
   */
  @Prop() styleId: 'two-row-66h' = 'two-row-66h';
  ...
  render() {
    return (
      <div class={`marquee ${this.styleId}`}>
        {this.items.map((item, i) => {
          return (
            <a
              href={item.link}
              class={`item ${i === 0 ? this.rollupClassName : ''} l-${
                this.items.length
              }`}
            >
              {this.styleId === 'two-row-66h' && [
  ...
  componentDidLoad() {
    if (this.styleId == 'two-row-66h' && this.items.length > 2) {
