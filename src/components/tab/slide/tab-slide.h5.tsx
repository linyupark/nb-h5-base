...
export class TabSlide {
   ...
   */
  @Prop() slideWH: string = '40|4';

  /**
   * 滑块是否带圆角
   */
  @Prop() slideRadius: boolean = false;

  get slideSize() {
    const [width, height]: any = this.slideWH.split('|');
    if (!width || !height) throw new Error('slideSize props error');
    return {
      width: `${width / 75}rem`,
      height: `${height / 75}rem`,
      borderRadius: this.slideRadius ? `${height / 2 / 75}rem` : 'none'
    };
  }
